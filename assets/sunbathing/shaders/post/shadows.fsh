...
// Directional shadow resolve pass (Polytone new post pipeline, classic depth).
//
...
//
// This variant assumes CLASSIC depth (sky/far = 1.0, NDC z in [-1,1]). The reversed-Z overlay ships
// its own copy for formats 88+ where the far plane clears to 0.0.
//
...
// surface they fall on (no oblique shadow-map texels). PolyShadowCamFract anchors the grid to
// absolute world space so it does not swim as the camera moves. 16 = block-sized; 32/48 = finer; 0 = off.
